...
type SimBin = {
  ...
  sumPayoff: number;
  medianPayoff: number;
  sumGammaTheta: number;
  medianGammaTheta: number;
};
...
type SimWorkerSuccess = {
  ...
  sampledGammaThetaBuffer: ArrayBuffer;
  bins: SimBin[];
  pathMin: number;
  pathMax: number;
  ...
  maxDrawdown: number;
  winRate: number;
  gammaThetaMin: number;
  gammaThetaMax: number;
};
  ...
  for (const leg of request.legs ?? []) {
    ...
    const expiryTs =
      pricing?.expirationTs != null && Number.isFinite(pricing.expirationTs)
        ...
        : null;
    const remainingSecondsStart =
      expiryTs != null ? Math.max(0, expiryTs - request.valuationTs) : 0;
    const remainingSeconds =
      expiryTs != null
        ? Math.max(0, remainingSecondsStart - request.horizonSeconds)
        : 0;
    optionLegs.push({
      sign,
      qty,
      ...
      iv,
      entryPrice,
      remainingYearsStart: remainingSecondsStart / SECONDS_PER_YEAR,
...
const simulate = (request: SimWorkerRequest): SimWorkerSuccess => {
  ...
  const gammaThetaPnls = new Float64Array(rows);
  const stepYears = request.params.dt;
  const stepDays = stepYears * 365.25;
  ...
  let wins = 0;
  let maxDrawdown = 0;
  let gammaThetaMin = Number.POSITIVE_INFINITY;
  let gammaThetaMax = Number.NEGATIVE_INFINITY;

  for (let r = 0; r < rows; r += 1) {
    let s = baseline;
    let peak = baseline;
    let worstDrawdown = 0;
    let gammaThetaTotal = 0;
    const stopHit =
    ...
    for (let c = 0; c < steps; c += 1) {
      const prevS = s;
      const z = randn();
      const nextS = prevS * Math.exp(drift + volStep * z);
      s = nextS;
      ...
      if (optionLegs.length > 0) {
        const elapsedYears = c * stepYears;
        const dS = nextS - prevS;
        for (const leg of optionLegs) {
          const timeToExpiry = Math.max(0, leg.remainingYearsStart - elapsedYears);
          ...
          const gammaPnl = 0.5 * greeks.gamma * dS * dS;
          const thetaPnl = greeks.theta * stepDays;
          gammaThetaTotal += leg.sign * leg.qty * (gammaPnl - thetaPnl);
        }
      }
    }
    ...
    gammaThetaPnls[r] = gammaThetaTotal;
    if (gammaThetaTotal < gammaThetaMin) gammaThetaMin = gammaThetaTotal;
    if (gammaThetaTotal > gammaThetaMax) gammaThetaMax = gammaThetaTotal;
  ...
  if (!Number.isFinite(gammaThetaMin)) gammaThetaMin = 0;
  if (!Number.isFinite(gammaThetaMax)) gammaThetaMax = 0;
  ...
  const gammaThetaSums = new Float64Array(histBins);
  const payoffLists: number[][] = Array.from({ length: histBins }, () => []);
  const gammaThetaLists: number[][] = Array.from(
    { length: histBins },
    () => [],
  );
  ...
  for (let i = 0; i < rows; i += 1) {
    ...
    counts[idx] += 1;
    sums[idx] += payoffs[i];
    gammaThetaSums[idx] += gammaThetaPnls[i];
    payoffLists[idx].push(payoffs[i]);
    gammaThetaLists[idx].push(gammaThetaPnls[i]);
  }
  ...
  for (let i = 0; i < histBins; i += 1) {
    ...
    bins[i] = {
      ...
      sumGammaTheta: gammaThetaSums[i],
      medianGammaTheta: medianOfArray(gammaThetaLists[i]),
    };
  }
  ...
  const sampledGammaTheta = new Float64Array(sampleRows);
  ...
  for (let i = 0; i < sampleRows; i += 1) {
    ...
    sampledGammaTheta[i] = gammaThetaPnls[pathIndex] ?? 0;
  }
  ...
  return {
    ...
    sampledGammaThetaBuffer: sampledGammaTheta.buffer,
    bins,
    pathMin,
    pathMax,
    finalPriceMin,
  ...
  while (queuedRequest != null) {
    ...
    try {
      ...
      scope.postMessage(response, [
        ...
        response.sampledGammaThetaBuffer,
      ]);
    } catch (error) {
