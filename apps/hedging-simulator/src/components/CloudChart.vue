...
type SimComputation = {
  ...
  sampledGammaTheta: Float64Array;
  bins: SimBin[];
  pathMin: number;
  pathMax: number;
  finalPriceMin: number;
...
const handleWorkerMessage = (
  ...
  const sampledGammaTheta = new Float64Array(payload.sampledGammaThetaBuffer);
  ...
  pending.resolve({
    ...
    gammaThetaMin: payload.gammaThetaMin,
    gammaThetaMax: payload.gammaThetaMax,
  });
};
...
const updateDynamicScene = (
  ...
  const negSpan = Math.abs(Math.min(gammaThetaMin, 0));
  const posSpan = Math.max(gammaThetaMax, 0);
  // Piecewise side scaling for gamma-theta:
  // negatives map across the lower half, positives across the upper half,
  // so each side keeps contrast even when tails are asymmetric.
  ...
  const negGammaThetaColorT = d3
    .scaleLinear()
    .domain([-Math.max(negSpan, 1e-9), 0])
    .range([resolvedMin, colorMid])
    .clamp(true);
  const posGammaThetaColorT = d3
    .scaleLinear()
    .domain([0, Math.max(posSpan, 1e-9)])
    .range([colorMid, resolvedMax])
    .clamp(true);
  const gammaThetaColorRamp = (value: number): string => {
    const t =
      value < 0
        ? negGammaThetaColorT(value)
        : value > 0
          ? posGammaThetaColorT(value)
          : colorMid;
    return d3.interpolateRdBu(t);
  };
  const cloudFillForPath = (pathIndex: number): string =>
    gammaThetaColorRamp(sampledGammaTheta[pathIndex] ?? 0);
  ...
  const smoothDisplayBins = (input: SimBin[]): SimBin[] => {
    ...
    for (let i = 1; i < output.length - 1; i += 1) {
      ...
      output[i].sumGammaTheta = lerp(
        output[left].sumGammaTheta,
        output[right].sumGammaTheta,
      );
      output[i].medianGammaTheta = lerp(
        output[left].medianGammaTheta,
        output[right].medianGammaTheta,
      );
    }
  ...
  const payoffSpan =
    Number.isFinite(maxAbsAvgPayoff) && maxAbsAvgPayoff > 0
      ? maxAbsAvgPayoff * 1.1
      : 1;
  const getAvgGammaTheta = (bin: SimBin): number =>
    bin.count > 0 ? bin.sumGammaTheta / bin.count : 0;
  const maxAbsAvgGammaTheta =
    d3.max(displayBins, (value) => Math.abs(getAvgGammaTheta(value))) || 1;
  const avgGammaThetaSpan =
    Number.isFinite(maxAbsAvgGammaTheta) && maxAbsAvgGammaTheta > 0
      ? maxAbsAvgGammaTheta * 1.1
      : 1;
  ...
  const xHist =
    histogramMode === "price"
      ...
      : histogramMode === "prob"
        ...
        : d3
            .scaleLinear()
            .domain([-avgGammaThetaSpan, avgGammaThetaSpan])
            .range([0, histogramWidth]);
  const xZero = xHist(0);
  ...
  const barFill = (bin: SimBin): string =>
    gammaThetaColorRamp(getAvgGammaTheta(bin));
  ...
  const barX = (bin: SimBin, progress: number): number => {
    ...
    const avgGammaTheta = getAvgGammaTheta(bin) * progress;
    const clamped = Math.min(
      avgGammaThetaSpan,
      Math.max(-avgGammaThetaSpan, avgGammaTheta),
    );
    const xValue = xHist(clamped);
    return Math.min(xZero, xValue);
  };
